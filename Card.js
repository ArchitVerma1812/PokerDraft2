...
class Card {
  ...
  displayCard(placeHolder,flipped=true) {
    ...
    this.placeHolder = this.card;
    //this.placeHolder.classList.add(card);
    this.flipped = flipped;
    if (flipped){
      this.placeHolder.backgroundPosition = -150*this.position + "px";
    }
  }
  
  flip(){
    if (this.flipped) {
      this.placeHolder.style.backgroundPosition = 0+"px";
      this.flipped = false;
    } else {
